...
include("../experiment_utils/experiments1D_with_transforms.jl")
...
import .ExperimentsWithTransforms: master_1D_experiment_with_transforms
...
# EXPERIMENT NAME
exp_name = "q2Potential_Dsinperturb1D_LMt"

# STEPSIZES
number_of_stepsizes = 23
stepsizes = 10 .^ range(-2.0, 0.2, length=number_of_stepsizes)

save_dir = "$(master_dir)/$(exp_name)"

master_1D_experiment_with_transforms(integrators, num_repeats, potential, diffusion, T, sigma, stepsizes, bin_boundaries, save_dir; chunk_size=chunk_size, x0=nothing, time_transform=time_transform, space_transform=space_transform)
