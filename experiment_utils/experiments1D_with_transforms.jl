module ExperimentsWithTransforms
...
function run_1D_experiment_with_transforms(integrator, num_repeats, V, D, T, sigma, stepsizes, probabilities, bin_boundaries, save_dir; chunk_size=10000000, x0=nothing, time_transform=false, space_transform=false, x_of_y=nothing)
    ...
    V, D = transform_potential_and_diffusion(V, D, sigma, time_transform, space_transform, x_of_y)
...
function run_ess_autocorrelation_experiment_with_transforms(integrator, num_repeats, V, D, T, sigma, stepsize, max_lag, save_dir; time_transform=false, space_transform=false, x_of_y=nothing)
    @assert max_lag > 0
...
function run_autocorrelation_experiment_with_transforms(integrator, num_repeats, V, D, T, sigma, stepsize, max_lag, save_dir; time_transform=false, space_transform=false, x_of_y=nothing)
